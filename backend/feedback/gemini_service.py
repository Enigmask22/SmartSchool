...
"""
    ...
    def create_cn_feedback_prompt(
        ...
        """Tạo prompt nhận xét CẢ NĂM (CN) — tổng kết năm học."""
        ...
        # Build subject table from subject_details
        subject_lines = ""
        ...
        prompt = f"""
Bạn là trợ lý AI của giáo viên chủ nhiệm. Hãy nhập vai giáo viên và viết nhận xét TỔNG KẾT CẢ NĂM ngắn gọn, chuyên nghiệp cho học sinh (không phải sinh viên) để gửi cho phụ huynh.
...
"""
    ...
    async def generate_cn_feedback(
        ...
        """Tạo nhận xét CẢ NĂM qua Gemini API."""
        ...
        if response and response.text:
            return response.text.strip()
        raise Exception("Empty response from Gemini for CN feedback")
