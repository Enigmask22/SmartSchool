...
@router.get("/recognition-logs", response_model=dict)
async def get_homeroom_recognition_logs(
    limit: int = Query(default=50, description="Số lượng logs cần lấy"),
    student_id: Optional[int] = Query(default=None, description="Filter theo student_id"),
    current_user=Depends(get_current_user),
    db=Depends(get_db)
):
    """Lấy recognition logs của học sinh trong lớp chủ nhiệm"""
    try:
        if not verify_homeroom_teacher(current_user):
            raise HTTPException(
                status_code=status.HTTP_403_FORBIDDEN,
        ...
        # Lấy danh sách student_ids của lớp chủ nhiệm
        homeroom_students = db.rpc("get_homeroom_students", {"p_user_id": current_user["id"]}).execute()
        student_ids = [s["student_id"] for s in homeroom_students.data or []]
        ...
        """).in_("student_id", student_ids).order("recognition_time", desc=True).limit(limit)
        ...
        return {
            "success": True,
            "message": "Lấy recognition logs thành công",
